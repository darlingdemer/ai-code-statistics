...
import {
  hasGitAiNativeCopilotHooksConfigured,
...
export class AIEditManager {
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    const nativeHooksConfigured = hasGitAiNativeCopilotHooksConfigured(
      chatConfig.get<boolean>("useHooks") === true,
      chatConfig.get<Record<string, boolean>>("hookFilesLocations"),
    );

    this.legacyCopilotHooksEnabled = !shouldSkipLegacyCopilotHooks(
      vscode.version,
      nativeHooksConfigured,
    );
    ...
    } else if (shouldSkipLegacyCopilotHooks(vscode.version) && !nativeHooksConfigured) {
      console.log(`[git-ai] AIEditManager: VS Code ${vscode.version} supports native hooks, but git-ai native Copilot hooks are not configured; keeping legacy extension checkpoints enabled`);
    }
