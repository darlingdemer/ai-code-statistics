...
/// Format: "stripe_live_abc123" -> "stri********c123"
...
mod tests {
    ...
    fn test_p_random_random_strings() {
        ...
        assert!(p_random(b"stripe_test_4eC39HqLyjWDarjtT1zdp7dc") > 1.0 / 1e4);
    }
    ...
    fn test_is_random() {
        ...
        assert!(is_random(b"stripe_test_4eC39HqLyjWDarjtT1zdp7dc"));
        assert!(is_random(b"AWSAIOSFODNN7EXAMPLE"));
    ...
    fn test_extract_tokens() {
        let text = "API_KEY=stripe_test_4eC39HqLyjWDarjtT1zdp7dc";
        let tokens = extract_tokens(text);
        ...
        assert!(
            tokens
                .iter()
                .any(|&(start, end)| &text[start..end] == "stripe_test_4eC39HqLyjWDarjtT1zdp7dc")
        );
    }
    ...
    fn test_redact_secret() {
        assert_eq!(
            redact_secret("stripe_test_4eC39HqLyjWDarjtT1zdp7dc"),
            "stri********p7dc"
        );
        assert_eq!(redact_secret("AWSAIOSFODNN7EXAMPLE"), "AWSA********MPLE");
        assert_eq!(redact_secret("short"), "*****"); // Too short
    }

    #[test]
    fn test_redact_secrets_in_text() {
        let text = "Set API_KEY=stripe_test_4eC39HqLyjWDarjtT1zdp7dc in your config";
        let (redacted, count) = redact_secrets_in_text(text);
        assert!(!redacted.contains("stripe_test_4eC39HqLyjWDarjtT1zdp7dc"));
        assert!(redacted.contains("stri********p7dc"));
        assert_eq!(count, 1);
    }
...
stripe_live_51HG8vDKj2xPmVnRqT9wYzABC and you should use it carefully.
...
"#;
        ...
        assert!(!redacted.contains("stripe_live_51HG8vDKj2xPmVnRqT9wYzABC"));
        assert!(redacted.contains("stri********zABC"));
        assert_eq!(count, 1);
...
fn main() {
    ...
    // API configuration
    let stripe_key = "stripe_test_4eC39HqLyjWDarjtT1zdp7dc";
    let aws_key = "AWSAIOSFODNN7EXAMPLE";
...
"#;
        ...
        assert!(!redacted.contains("stripe_test_4eC39HqLyjWDarjtT1zdp7dc"));
        assert!(!redacted.contains("AWSAIOSFODNN7EXAMPLE"));
        assert_eq!(count, 3);
...
STRIPE_SECRET_KEY=stripe_live_51HG8vDKj2xPmVnRqT9wYzABC
AWS_ACCESS_KEY_ID=AWSAIOSFODNN7EXAMPLE
