    ...
    fn apply_notify(config: &Value, notify_args: &[String]) -> Result<Value, GitAiError> {
        ...
        let merged_notify = match Self::notify_args_from_config(config) {
            Some(existing_notify) if Self::is_git_ai_codex_notify_args(&existing_notify) => {
                let mut merged_notify = notify_args.to_vec();
                if let Some(existing_binary_path) = existing_notify.first()
                    && existing_binary_path == &notify_args[0]
                {
                    merged_notify[0] = existing_binary_path.clone();
                }
                merged_notify
            }
...
mod tests {
    ...
    #[test]
    fn test_apply_notify_upgrades_legacy_notify_args_to_stdin() {
...
"#,
    ...
    #[test]
    fn test_apply_notify_updates_binary_path_and_sets_stdin() {
        let existing = CodexInstaller::parse_config_toml(
            r#"
...
"#,
    ...
    #[test]
    #[serial]
    fn test_install_hooks_upgrades_legacy_notify_args_to_stdin() {
        with_temp_home(|home| {
            let codex_dir = home.join(".codex");
            fs::create_dir_all(&codex_dir).unwrap();
            let config_path = codex_dir.join("config.toml");
...
"#,
            ...
            assert!(
                result.is_some(),
                "install should rewrite legacy notify args to use stdin"
            );
    ...
    #[test]
    #[serial]
    fn test_install_hooks_updates_binary_path_and_sets_stdin() {
        with_temp_home(|home| {
            let codex_dir = home.join(".codex");
            fs::create_dir_all(&codex_dir).unwrap();
            let config_path = codex_dir.join("config.toml");
...
"#,
            ...
            assert!(
                result.is_some(),
                "install should update stale binary path and use stdin"
            );
